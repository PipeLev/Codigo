...
diccionario = { # es un diccionario para guardar multiples listas
    ...
    "co̱o": ("sexto", "roberto", 12)} # y esta es la segunda
...
duda = input("aqui se ingresa un texto") #se ingresara un texto o nada pero abajo se puede escribir algo y lo guardara como una variable de texto y se tratara como tal

numero = int(input("aqui ingresar un numero")) #esto sirve para pasar lo escrito como un numero que se tratara como tal

decimales = float(input("ingrese tu numero con decimal")) #para decimal
